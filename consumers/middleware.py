"""Build Cache — middleware for eviction payloads."""
...
class BuildMiddleware:
    """Middleware for Build Cache eviction payloads."""
...
def parse_evictions(payload: str) -> List[Dict[str, Any]]:
    ...
    return [BuildMiddleware._coerce(item) for item in raw]


def store_eviction_to_str(
    record: Dict[str, Any], indent: Optional[int] = None
) -> str:
    """Convenience wrapper — serialise a Eviction to a JSON string."""
    if indent is None:
        return BuildMiddleware.dumps(record)
